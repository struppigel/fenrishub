...
class UploadedLogDetailViewTests(UploadedLogSharedSetupMixin, TestCase):
    ...
    def test_rename_forum_username_username_on_upload_detail(self):
        uploaded = UploadedLog.objects.create(
            upload_id='rename-test',
            forum_username='old_name',
            original_filename='x.txt',
            content='payload',
        )
        self.client.login(username='alice', password='password123')
        ...
        response = self.client.post(
            reverse('view_uploaded_log', args=[uploaded.upload_id]),
            {'action': 'rename_forum_username', 'forum_username': 'new_name'},
        )

        self.assertEqual(response.status_code, 302)
        uploaded.refresh_from_db()
        self.assertEqual(uploaded.forum_username, 'new_name')

    def test_rename_forum_username_ignores_empty_username(self):
        uploaded = UploadedLog.objects.create(
            upload_id='rename-empty',
            forum_username='keep_this',
            original_filename='x.txt',
            content='payload',
        )
        self.client.login(username='alice', password='password123')
        ...
        self.client.post(
            reverse('view_uploaded_log', args=[uploaded.upload_id]),
            {'action': 'rename_forum_username', 'forum_username': ''},
        )

        uploaded.refresh_from_db()
        self.assertEqual(uploaded.forum_username, 'keep_this')
